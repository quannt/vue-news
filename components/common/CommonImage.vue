...
export default {
  ...
  // Could not make this work with nuxt.config.js so I'm using the code from the plugin directly here
  ...
  created() {
    this.src =
      this.$slots.image[0].data.attrs.src ||
      this.$slots.image[0].data.attrs['data-src']
    if (this.src) {
  ...
  updated() {
    const receivedSrc =
      this.$slots.image[0].data.attrs.src || this.$slots.image[0].data.attrs
